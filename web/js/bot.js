...
$(".bot-view" + this.id + " .bot-happiness").html(bot.happiness.toFixed(2) + evalToEmoji(bot.happiness));
	...
	toSVGPortrait: function() {
		var headR = portraitSize * .36;
		var r0 = headR * (.7 - .4 * this.dna[9]);
		var r1 = headR * 1.2;
		var r2 = headR * 1.1;
		var tilt = -.3 + .6 * utilities.noise(20 * this.dna[2] + 20);
		var subR = r0 * .9;
		var outerTilt0 = -.4 + .8 * utilities.noise(20 * this.dna[0]);
		var outerTilt1 = -.4 + .8 * utilities.noise(20 * this.dna[1] + 10);
		...
		var b0 = Vector.polar(r2 * .8, tilt + theta0);
		var b1 = Vector.polarOffset(p2, r2 * (.3 + .3 * this.dna[5]), tilt + theta0)
		var cb0 = Vector.polarOffset(b1, subR * .8, tilt + theta0 - Math.PI / 2)
		var cb1 = Vector.polarOffset(b1, subR * .8, tilt + theta0 + Math.PI / 2)
