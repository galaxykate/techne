...
var tracery = function() {
	...
	TraceryNode.prototype.expandChildren = function(childRule, preventRecursion) {
		...
		if (this.childRule !== undefined) {
			...
			console.error("No child rule provided, can't expand children");
		}
	};
	...
	TraceryNode.prototype.expand = function(preventRecursion) {
		...
		if (!this.isExpanded) {
			...
			case 1:
				...
				if (selectedRule === null || selectedRule === undefined) {
					console.warn("No selected rule");
				}
	...
	RuleSet.prototype.selectRule = function(errors) {
		...
		console.warn("No default rules defined for " + this);
		return null;
	...
	var Symbol = function(grammar, key, rawRules) {
		...
		// Wrap
		if (typeof rawRules == 'string' || rawRules instanceof String)
			this.rawRules = [rawRules];
	...
	Grammar.prototype.selectRule = function(key, node, errors) {
		if (this.symbols[key]) {
			...
			if (rule !== undefined && rule !== null)
				return rule;
			console.log(key, this.symbols[key]);
		}
